> This repo is just my re-frame playground
> aim to famaliar this framework.
...
* [`src/cljs/howard/reframe_playground/`](src/cljs/howard/reframe_playground/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/howard/reframe_playground/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `reframe-playground` project root directory
...
The `debug?` variable in [`config.cljs`](src/howard/reframe_playground/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns howard.reframe-playground.example
  (:require [howard.reframe-playground.config :as config])
